...
stdenv.mkDerivation rec {
  ...
  buildInputs = [
    vulkan-headers
    vulkan-utility-libraries
  ] ++ lib.optionals (stdenv.isLinux) [ libX11 libXau libXdmcp libXrandr libffi libxcb wayland ];

  cmakeFlags = [
    "-DGLSLANG_INSTALL_DIR=${glslang}"
    "-DSPIRV_HEADERS_INSTALL_DIR=${spirv-headers}"
    "-DSPIRV-Headers_DIR=${spirv-headers}/share/cmake/SPIRV-Headers"
    ...
    "-Wno-dev"
  ] ++ lib.optionals (stdenv.isLinux) [ "-DPKG_CONFIG_EXECUTABLE=${pkg-config}/bin/pkg-config" ];
